...
typedef BaseFunction<T extends BaseBloc> = void Function(
    T? bloc, Set<StreamSubscription?>? subscriptions);
...
class BaseProvider<T extends BaseBloc> extends StatefulWidget {
  ...
  BaseProvider._copy({
    ...
    required BaseInherited<T> inherited,
          ...
          .._bloc = _instances[key]!._inherited._bloc as T
  ...
  static BaseBloc bloc<R extends BaseInherited>(BuildContext context) =>
      (context.dependOnInheritedWidgetOfExactType<R>() as R)._bloc;

  static PublishSubject<BaseException?>? exception<R extends BaseInherited>(
          BuildContext context) =>
      (context.dependOnInheritedWidgetOfExactType<R>() as R).exception;
  ...
  static Future<dynamic> _dispose(Key key) async {
    if (_instances[key] != null) {
      List<Future> futures = [];
      futures
          .add(_instances[key]!._inherited._bloc.dispose() ?? Future.value());
      futures.add(
          _instances[key]!._inherited.exception?.close() ?? Future.value());
      if (_instances[key]?._subscriptions != null) {
        for (StreamSubscription? subscription
            in _instances[key]!._subscriptions)
          futures.add(
            subscription?.cancel() ?? Future(() => null),
          );
      }
